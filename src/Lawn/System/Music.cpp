...
bool Music::TodLoadMusic(MusicFile theMusicFile, const std::string& theFileName)
{
	...
	bool ok = mApp->mMusicInterface->LoadMusicFromMemory((int)theMusicFile, aData, (size_t)aSize);
...
#ifdef __EMSCRIPTEN__
	(void)theVolume;
	bool aNoLoop = (mCurMusicTune == MusicTune::MUSIC_TUNE_CREDITS_ZOMBIES_ON_YOUR_LAWN &&
		theMusicFile == MusicFile::MUSIC_FILE_CREDITS_ZOMBIES_ON_YOUR_LAWN);
	mMusicInterface->PlayMusic((int)theMusicFile, theOffset, aNoLoop);
#else
	SDLMusicInterface* anSDL = (SDLMusicInterface*)mApp->mMusicInterface;
	auto anItr = anSDL->mMusicMap.find((int)theMusicFile);
	TOD_ASSERT(anItr != anSDL->mMusicMap.end());
...
unsigned long Music::GetMusicOrder(MusicFile theMusicFile)
{
	...
	return (unsigned long)mApp->mMusicInterface->GetMusicOrder((int)theMusicFile);
}
...
void Music::UpdateMusicBurst()
{
	...
	{
		mMusicInterface->SetSongVolume((int)mCurMusicFileMain, aMainTrackVolume);
		mMusicInterface->SetSongVolume((int)mCurMusicFileDrums, aDrumsVolume);
#ifndef __EMSCRIPTEN__
...
#ifdef __EMSCRIPTEN__
			if (mCurMusicFileMain != MusicFile::MUSIC_FILE_NONE)
				mMusicInterface->ResumeMusic((int)mCurMusicFileMain);
			if (mCurMusicFileDrums != MusicFile::MUSIC_FILE_NONE)
				mMusicInterface->ResumeMusic((int)mCurMusicFileDrums);
			if (mCurMusicFileHihats != MusicFile::MUSIC_FILE_NONE)
				mMusicInterface->ResumeMusic((int)mCurMusicFileHihats);
#else
