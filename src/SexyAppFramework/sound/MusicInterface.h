...
public:
	...
	// Optional: load from memory (for PAK/streaming). Default returns false.
	virtual bool			LoadMusicFromMemory(int theSongId, const void* theData, size_t theSize) { (void)theSongId; (void)theData; (void)theSize; return false; }
	...
	// MOD/track order (for layered music sync); -1 if not supported
	virtual int				GetMusicOrder(int theSongId) { return -1; }
};
}
